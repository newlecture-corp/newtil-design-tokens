...
## 0.2.6 (2026-09-18) — 틴트 규칙: 바닥은 깊게, 그 위는 같은 계열의 밝은 색

- `--color-primary`: 씨앗의 색상·채도는 그대로 두고 명도만 "채우는 바닥"이 될 만큼(l ≤ 0.53)으로 낮춘다. 연두·주황처럼 밝은 씨앗은 버튼·스위치·배지·FAB 바닥이 한 단계 깊어지고, 로고·링크·outlined 글자도 흰 바탕에서 4.5:1 이 넘는다(전엔 2.6). 어두운 씨앗(#398526 기본값, NCafe 갈색, 뉴렉처 인디고)은 그대로.
- `--color-on-primary`: 바닥과 같은 색상의 아주 밝은 색(l 0.97, 채도 조금). 순백보다 부드럽고 어느 씨앗이든 바닥과 한 계열. 0.2.3~0.2.5 의 "밝으면 검정/진초록" 규칙은 대비는 맞췄지만 색이 따로 놀아 폐기.
- `--color-primary-hover`·`-active`: 깎인 primary 에서 다시 만든다(전엔 씨앗 램프라 hover 가 primary 보다 밝아질 수 있었다).
- 다크: 바닥은 밝게(l ≥ 0.72), 그 위는 같은 색상의 아주 어두운 색(l 0.22). 씨앗의 밝은 기운은 `primary-subtle`·연한 자리가 받는다.
- `check.mjs`: min·max·clamp 속 calc, 여러 줄 값 계산. 모든 씨앗에서 on-primary/primary 4.5 이상 확인(#5cb35a 4.55, #8cba35 4.61, #f97316 5.11, #398526 4.62).
